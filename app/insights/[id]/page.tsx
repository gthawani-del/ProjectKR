...
export default async function InsightPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const relatedSectorIds = relatedSectors.map((sector) => sector.id);
  const relatedMatters = matters.filter((matter) => relatedSectorIds.includes(matter.sectorId));
      ...
      <main>
        ...
        <section className="internal-section">
          <div className="section-grid">
            <div><div className="section-label">01 / Connected matters</div><h2>What this signal can affect.</h2></div>
            <div className="related-links">{relatedMatters.map((matter)=><Link href={`/matters/${matter.id}`} key={matter.id}><strong>{matter.name}</strong><br/><span>{matter.intro}</span></Link>)}</div>
          </div>
        </section>

        <section className="internal-section">
          <div className="section-grid">
            <div><div className="section-label">02 / Practice Areas</div><h2>Capabilities connected to the development.</h2></div>
            <div className="related-links">{relatedPractices.map((practice)=><Link href={`/practices/${practice.id}`} key={practice.id}><strong>{practice.name}</strong><br/><span>{practice.summary}</span></Link>)}</div>
          </div>
        </section>

        <section className="internal-section">
          <div className="section-grid">
            <div><div className="section-label">03 / Connected sectors</div><h2>From signal to market context.</h2></div>
