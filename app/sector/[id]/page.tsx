...
export default async function SectorPage({params}:{params:Promise<{id:string}>}){
 ...
 return <InternalShell><main>
  ...
  <section className="internal-section"><div className="section-grid"><div><div className="section-label">02 / People in context</div><h2>Relevant people, not a separate directory.</h2></div><div className="people-context">{relatedLawyers.map((lawyer)=><article className="person-node" key={lawyer.id}>{peopleImages[lawyer.id]?<div className="lawyerPortrait"><Image src={peopleImages[lawyer.id]} alt={lawyer.name} fill sizes="(max-width: 768px) 88vw, 28vw" /></div>:<div className="initials">{lawyer.initials}</div>}<h3>{lawyer.name}</h3><p>{lawyer.role}</p><div className="tag-list">{lawyer.worksAcross.slice(0,3).map((item)=><span className="tag" key={item}>{item}</span>)}</div><Link href={`/people/${lawyer.id}`}>View profile →</Link></article>)}</div></div></section>
